  ...
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-table v-loading="loading" :data="tableData" border style="width: 100%" class="taba">
        ...
        <el-table-column prop="sex" label="性别" :formatter="sexStatus" />
        <el-table-column prop="shelfTime" label="上架时间" :formatter="formDate" />
        <el-table-column prop="region" label="地区" />
        <el-table-column prop="budget" label="预算" />
        <el-table-column prop="apartment" label="户型" />
        <el-table-column prop="preOrderedProducts" label="预购产品" />
        <el-table-column prop="preorderTime" label="预购时间" :formatter="preorderFormDate" />
      ...
      <!--分页 start-->
      <div class="block">
        <el-pagination background :current-page.sync="currentPage2" :page-sizes="[10, 20, 30]" :page-size="pageSize" layout="sizes, prev, pager, next" :total="pageTotal" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
      </div>
      <!--分页 end-->
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    formDate(row) {
      return row.shelfTime.substring(0, 10)
    },
    // 预购事件
    preorderFormDate(row) {
      return row.preorderTime.substring(0, 10)
    },
    // 获取品牌列表
    getInfo() {
      const parmas = {
